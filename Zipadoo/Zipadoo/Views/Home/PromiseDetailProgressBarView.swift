    ...
    var body: some View {
        VStack(alignment: .leading) {
            ...
            VStack(alignment: .leading) {
                ...
                /*MyProgressSubView(myLocation: locationStore.myLocation, isShowingFriendSheet: $isShowingFriendSheet, region: $region, promise: promise, progressTrigger: $progressTrigger, promiseFinishCheck: $promiseFinishCheck)*/
//                
...
////                        @State var ratio = caculateRatio(location: friends.location)
//                        /// ratio에 맞춰서 이미지 위치 조정 다르게 변경
//        
////                        .onChange(of: isRefresh) {
////                            ratio = caculateRatio(location: locationStore.myLocation)
////                        }
//                        ProgressSubView(friends: friends, isShowingFriendSheet: $isShowingFriendSheet, region: $region, promise: promise, progressTrigger: $progressTrigger, promiseFinishCheck: $promiseFinishCheck)
//                    }
//                }
                /*
                /// 남은거리 / 전체거리 비율
                @State var ratio: Double = caculateRatio(location: locationStore.myLocation)
                /// ratio에 맞춰서 이미지 위치 조정 다르게 변경
                var offsetX: Double {
                    if ratio <= 0.3 {
                        // 0.3보다 작거나 같을때는 바끝에
...
//        let departureLatitude = 37.589521265258746
//        let departureLongitude = 127.05835612750708
        let totalDistance = straightDistance(x1: 37.589521265258746, y1: 127.05835612750708, x2: promise.latitude, y2: promise.longitude)
        ...
        print("총 계산 비율 : \(distanceRatio(depature: remainingDistance, arrival: totalDistance))")
...
//                }
        ...
        .onAppear {
            Task {
                ratio = caculateRatio(location: myLocation)
                print("\(AuthStore.shared.currentUser?.nickName ?? "나")ratio: \(ratio)")
            }
        }
//        .task {
    ...
    var body: some View {
        ...
        .onAppear {
            Task {
                ...
                print("\(friends.nickname)ratio: \(ratio)")
            }
        }
//        .task {
//                withAnimation(.easeIn) {
